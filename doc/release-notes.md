Online Core version 0.11.0 is now available from:

  <https://online.org/bin/online-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Online Core or other software:
...
related to the blockchain in the online system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Online Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Online Core will 
...
- #5334 `cf87536` libonlineconsensus: Add pkg-config support
...
- #5729 `9c4a5a5` Allow unit changes for read-only OnlineAmountField
- #5753 `0f44672` Add online logo to about screen
...
- #5809 `46bfbe7` Add online-cli man page
...
- #5849 `41259ca` contrib/init/onlined.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Online-Qt.app to Online-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for online-tx)
