...
Clone the git repositories for online and gitian.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/online-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/online-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/online-osx-build.assert
```
