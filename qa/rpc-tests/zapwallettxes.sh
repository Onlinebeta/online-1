...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars ONLINED and ONLINECLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

ONLINED=${ONLINED:-${1}/onlined}
CLI=${ONLINECLI:-${1}/online-cli}
...
$ONLINED -zapwallettxes=1 $B1ARGS &
B1PID=$!
$ONLINED -zapwallettxes=2 $B2ARGS &
B2PID=$!
